...
fn main() {
    let total = env_u32("UNIRUST_SCALE_COUNT", 200_000);
    let batch = env_u32("UNIRUST_SCALE_BATCH", 50_000).max(1);
    ...
    let shared_name = store.interner_mut().intern_value("John Doe");
    let shared_email = store.interner_mut().intern_value("john@example.com");
    let shared_phone = store.interner_mut().intern_value("555-1234");
    let shared_ssn = store.interner_mut().intern_value("123-45-6789");
    ...
    while offset <= total {
        ...
        if let Err(err) = unirust.stream_records(records) {
            eprintln!("scale bench failed: {err}");
            std::process::exit(1);
        }
        offset += count;
    }
