...
use crate::model::{RecordId, AttrId, ValueId, Record, KeyValue};
use crate::temporal::Interval;
use crate::dsu::TemporalGuard;
use crate::ontology::{Ontology, IdentityKey};
use crate::store::Store;
use crate::dsu::{TemporalDSU, MergeResult, Clusters};
use anyhow::Result;
...
impl OptimizedLinker {
    ...
    fn link_records_optimized(&mut self, store: &Store, ontology: &Ontology) -> Result<LinkingResult> {
        ...
        for block in &blocks {
            for i in 0..block.record_ids.len() {
                for j in (i + 1)..block.record_ids.len() {
                    ...
                    if let Some(ConflictResolution::Unresolvable) = self.check_strong_identifier_conflict(store, ontology, record_a_id, record_b_id, Interval::new(0, 1).unwrap()) {
                        unresolvable_conflicts.insert((record_a_id, record_b_id));
                        unresolvable_conflicts.insert((record_b_id, record_a_id)); // Add both directions
                    }
                }
            }
        }
        ...
        for edge in all_edges {
            ...
            if self.would_create_conflict_in_clusters(store, ontology, &mut dsu, edge.record_a, edge.record_b, edge.interval) {
                let suppressed = SuppressedMerge::new(
                    edge.record_a,
                    edge.record_b,
                    edge.interval,
        ...
        if self.should_apply_conflict_splitting(store, ontology) {
            result.clusters = self.split_clusters_with_unresolvable_conflicts(store, ontology, result.clusters)?;
        }
    ...
    fn create_identity_blocks(&self, store: &Store, ontology: &Ontology) -> Result<Vec<IdentityBlock>> {
    ...
    fn generate_edges_for_block(
        ...
        let edges: Vec<ShouldLinkEdge> = record_pairs
            ...
            .filter_map(|&(i, j)| {
                ...
                if let (Some(record_a), Some(record_b)) = (store.get_record(record_a_id), store.get_record(record_b_id)) {
    ...
    fn extract_key_values(&self, record: &Record, identity_key: &IdentityKey) -> Option<Vec<KeyValue>> {
    ...
    fn get_strong_identifier_descriptor<'a>(&self, record: &'a Record, strong_id: &crate::ontology::StrongIdentifier) -> Option<&'a crate::model::Descriptor> {
        for descriptor in &record.descriptors {
    ...
    fn temporal_intervals_overlap(&self, interval_a: &crate::temporal::Interval, interval_b: &crate::temporal::Interval) -> bool {
    ...
    fn would_create_conflict_in_clusters(
        ...
        if let Some(ConflictResolution::Unresolvable) = self.check_strong_identifier_conflict(store, ontology, record_a.id, record_b.id, _interval) {
    ...
    fn split_clusters_with_unresolvable_conflicts(
            ...
            for i in 0..cluster.records.len() {
                for j in (i + 1)..cluster.records.len() {
                    ...
                    if let Some(ConflictResolution::Unresolvable) = self.check_strong_identifier_conflict(
                        store, ontology, record_a, record_b, Interval::new(0, 1).unwrap()
                    ) {
            ...
            if has_unresolvable_conflicts {
                ...
                for &record_id in &cluster.records {
                    if !used_records.contains(&record_id) {
                        let cluster_id = crate::model::ClusterId(new_clusters.len() as u32 + 1);
                        new_clusters.push(crate::dsu::Cluster::new(cluster_id, record_id, vec![record_id]));
                    }
                }
            } else {
        ...
        Ok(Clusters { clusters: new_clusters })
    }
            ...
            for i in 0..records.len() {
                for j in (i + 1)..records.len() {
                    if let Some(conflict_resolution) = self.check_strong_identifier_conflict(
                        store, ontology, records[i].id, records[j].id, Interval::new(0, 1).unwrap()
                    ) {
                        _total_conflicts += 1;
